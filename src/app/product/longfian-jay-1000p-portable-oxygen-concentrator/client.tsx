...
import { Check, ShieldCheck, Truck, RotateCcw, Package, Zap, ChevronRight, Star } from 'lucide-react';
...
function StarRow({ r }: { r: number }) {
  return (
    <div style={{ display: 'flex', gap: 2 }}>
      {[1,2,3,4,5].map(i => (
        <Star key={i} style={{ width: 14, height: 14, fill: i <= Math.round(r) ? G : '#dde8dd', color: i <= Math.round(r) ? G : '#dde8dd' }} />
      ))}
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ position: 'relative', width: '100%', paddingBottom: '100%', background: '#edf6ec', borderRadius: 14, overflow: 'hidden', border: `2px solid #e8f0e8`, boxShadow: '0 4px 24px rgba(61,170,53,0.10)' }}>
        <Image src={GALLERY[main]} alt={PNAME} fill style={{ objectFit: 'contain', padding: 24 }} sizes="(max-width:768px) 100vw, 50vw" priority />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginTop: 10 }}>
        {GALLERY.map((src, i) => (
          <button key={i} onClick={() => setMain(i)} style={{ display: 'block', position: 'relative', width: '100%', paddingTop: '100%', border: `2px solid ${i === main ? G : '#e8f0e8'}`, borderRadius: 8, overflow: 'hidden', opacity: i === main ? 1 : 0.55, cursor: 'pointer', background: '#edf6ec', transition: 'all 0.2s' }}>
...
function Label({ text }: { text: string }) {
  return <span style={{ fontSize: 11, letterSpacing: '0.22em', textTransform: 'uppercase', color: G, fontWeight: 600, display: 'block', marginBottom: 12 }}>◆ {text}</span>;
}
...
function VideoEmbed({ id, title, dark }: { id: string; title: string; dark?: boolean }) {
  return (
    <div style={{ position: 'relative', width: '100%', paddingBottom: '56.25%', height: 0, overflow: 'hidden', borderRadius: 12, border: dark ? '2px solid rgba(255,255,255,0.08)' : '2px solid #e8f0e8', boxShadow: dark ? 'none' : '0 4px 20px rgba(61,170,53,0.09)' }}>
      <iframe src={`https://drive.google.com/file/d/${id}/preview`} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', border: 0 }} allow="autoplay; fullscreen; encrypted-media" allowFullScreen title={title} loading="lazy" />
    </div>
  );
}
...
export default function Jay1000PClient() {
  ...
  const cartItem = { id: PID, name: PNAME, price: PRICE.toString(), regular_price: MRP.toString(), images: GALLERY.map(src => ({ src })) };

  const doAdd = () => { setAdding(true); addToCart(cartItem); toast({ title: 'Added to Cart', description: `${PNAME} added to your cart.` }); setTimeout(() => setAdding(false), 600); };
    ...
    <div style={{ minHeight: '100vh', background: BG, overflowX: 'hidden', maxWidth: '100vw' }}>
      ...
      <section style={{ background: '#fff', paddingBottom: 'clamp(32px,5vw,56px)' }}>
        <div className="hero-wrap" style={{ maxWidth: W, margin: '0 auto', padding: `clamp(24px,5vw,40px) ${PAD} 0`, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(24px,5vw,64px)', alignItems: 'start' }}>
          ...
          {/* Gallery */}
          <div className="hero-gallery" style={{ position: 'sticky', top: 24, alignSelf: 'start' }}>
            <Gallery />
          </div>
          ...
          <div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
              {[
                { label: 'Portable O₂ Concentrator', g: false },
                { label: 'Flight Safe', g: true },
                { label: `${DISCOUNT}% OFF`, r: true },
              ].map((b, i) => (
                <span key={i} style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', padding: '5px 12px', borderRadius: 4, background: b.r ? '#E8175D' : b.g ? G : 'rgba(61,170,53,0.1)', color: (b.r || b.g) ? '#fff' : G, border: (!b.r && !b.g) ? '1.5px solid rgba(61,170,53,0.3)' : 'none' }}>
                  {b.label}
                </span>
              ))}
            </div>
            ...
            <button onClick={() => reviewsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
              <StarRow r={4.9} />
              <span style={{ fontSize: 12, color: 'rgba(15,17,23,0.5)', borderBottom: '1px solid rgba(15,17,23,0.2)' }}>4.9 (47 Reviews)</span>
            </button>

            <p style={{ fontSize: 14, color: 'rgba(15,17,23,0.6)', lineHeight: 1.75, marginBottom: 20 }}>
              FDA & FAA approved portable oxygen concentrator — ultra-light 1.98 kg, fly on any commercial airline, all-day battery life.
            </p>

            <div style={{ marginBottom: 24, display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                'Ultra-portable at just 1.98 kg — lighter than most laptops',
                'FDA & FAA approved — fly on any commercial airline worldwide',
                'Up to 11 hours battery — full day without a power outlet',
                'CDSCO approved for India — 2-year warranty by Sachdeva Medline',
              ].map((b, i) => (
                <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <span style={{ width: 18, height: 18, background: 'rgba(61,170,53,0.1)', border: `1.5px solid ${G}`, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2 }}>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
              <button onClick={doAdd} disabled={adding} style={{ flex: 1, padding: '14px 20px', background: '#fff', color: DARK, border: '2px solid #e8f0e8', borderRadius: 10, fontSize: 12, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', transition: 'all 0.15s' }}>
                {adding ? 'ADDED ✓' : 'ADD TO CART'}
              </button>
              <button onClick={doBuy} disabled={buying} style={{ flex: 1, padding: '14px 20px', background: G, color: '#fff', border: 'none', borderRadius: 10, fontSize: 12, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: 'inherit', boxShadow: '0 4px 14px rgba(61,170,53,0.35)', transition: 'all 0.15s' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: DARK, marginBottom: 20, padding: '10px 14px', background: 'rgba(61,170,53,0.06)', border: '1.5px solid rgba(61,170,53,0.2)', borderRadius: 8 }}>
              <Truck style={{ width: 15, height: 15, color: G, flexShrink: 0 }} />
            ...
            <div className="trust-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {[
                { icon: ShieldCheck, title: '100% Genuine', sub: 'Authorised Longfian dealer' },
                { icon: RotateCcw, title: 'Easy Returns', sub: '7-day return policy' },
                { icon: Package, title: '2-Year Warranty', sub: 'Full manufacturer warranty' },
                ...
                <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '10px 12px', background: '#fff', border: '1.5px solid #e8f0e8', borderRadius: 8 }}>
                  <item.icon style={{ width: 14, height: 14, color: G, flexShrink: 0, marginTop: 2 }} />
                  <div>
      ...
      {/* ──── STATS STRIP ──── */}
      <section style={{ background: G, padding: 'clamp(28px,5vw,40px) 0' }}>
        <div className="stats-grid" style={{ maxWidth: W, margin: '0 auto', padding: `0 ${PAD}`, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0 }}>
          {[
            ...
            <div key={i} style={{ textAlign: 'center', padding: 'clamp(0px,2vw,20px)', borderRight: i < 3 ? '1px solid rgba(255,255,255,0.2)' : 'none' }}>
              <p style={{ fontSize: 'clamp(22px,3vw,38px)', fontWeight: 900, color: '#fff', letterSpacing: '-0.02em', lineHeight: 1, marginBottom: 4 }}>{s.val}</p>
              <p style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.9)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 2 }}>{s.lbl}</p>
      ...
      <section style={{ background: '#fff', padding: `${VPAD} 0` }}>
        <div style={{ maxWidth: W, margin: '0 auto', padding: `0 ${PAD}` }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <H2>WHY CHOOSE THE<br /><span style={{ color: G }}>JAY-1000P?</span></H2>
          </div>
          <div className="benefits-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
            {[
              { icon: '✈️', title: 'Fly Anywhere', desc: 'FDA & FAA approved for all commercial airlines worldwide. No extra documentation — just pack and board.' },
              ...
              { icon: '🔋', title: '11-Hour Battery', desc: 'Double battery delivers up to 11 hours at Setting 1. Single battery gives 5.5 hours — full day usability.' },
              { icon: '💧', title: '93%±3% Pure O₂', desc: 'PSA molecular sieve technology delivers clinically accurate oxygen every breath, every time.' },
              { icon: '🔇', title: 'Whisper Quiet', desc: 'At ≤48 dB, quieter than a normal conversation. Use it in meetings, flights, or while sleeping.' },
              { icon: '🇮🇳', title: 'CDSCO Approved', desc: "Officially registered with India's drug regulatory authority. Sold by Sachdeva Medline with 25 years of experience." },
              ...
              <div key={i} style={{ padding: 'clamp(16px,2.5vw,24px)', background: BG, borderRadius: 12, border: '1.5px solid #e8f0e8' }}>
                <div style={{ fontSize: 32, marginBottom: 14 }}>{b.icon}</div>
                <h3 style={{ fontSize: 16, fontWeight: 800, color: DARK, marginBottom: 8 }}>{b.title}</h3>
                <p style={{ fontSize: 13, color: 'rgba(15,17,23,0.6)', lineHeight: 1.7 }}>{b.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ position: 'relative', width: '100%', overflow: 'hidden', lineHeight: 0 }}>
        <div style={{ position: 'relative', width: '100%', paddingBottom: 'clamp(28%,20vw,32%)' }}>
      ...
      <section style={{ background: DARK, padding: `${VPAD} 0` }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: `0 ${PAD}` }}>
          ...
          <H2 light>JAY-1000P —<br /><span style={{ color: G }}>See It In Action</span></H2>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.55)', lineHeight: 1.8, marginBottom: 32, maxWidth: 520 }}>
            The complete product video — design, features, and how the JAY-1000P fits seamlessly into daily life for COPD, travel, and more.
          </p>
      ...
      <section style={{ background: BG, padding: `${VPAD} 0` }}>
        <div style={{ maxWidth: W, margin: '0 auto', padding: `0 ${PAD}` }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <H2>TECHNICAL<br /><span style={{ color: G }}>SPECIFICATIONS</span></H2>
          </div>
          <div className="specs-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, maxWidth: 900, margin: '0 auto' }}>
            {[
              ['Oxygen Purity', '93% ± 3%'],
              ['Flow Settings', '1–5 Levels (Pulse Dose)'],
              ['Weight', '1.98 kg'],
              ['Dimensions', '183 × 86 × 199 mm'],
              ['Battery Life — Single', '5.5 hrs at Setting 1 / 3 hrs at Setting 2'],
              ['Battery Life — Double', 'Up to 11 hrs at Setting 1'],
              ...
              ['Warranty', '2 Years — Sachdeva Medline'],
              ['Manufacturer', 'Longfian Scitech Co., Ltd'],
            ].map(([lbl, val], i) => (
              <div key={i} style={{ display: 'flex', gap: 16, padding: '14px 18px', background: i % 2 === 0 ? '#fff' : 'rgba(61,170,53,0.04)', border: '1.5px solid #e8f0e8', borderRadius: 8 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'rgba(15,17,23,0.4)', minWidth: 130, flexShrink: 0, lineHeight: 1.5 }}>{lbl}</span>
      ...
      <section style={{ background: '#fff', padding: `${VPAD} 0` }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: `0 ${PAD}` }}>
          ...
          <H2>MEDICA 2025 —<br /><span style={{ color: G }}>FDA & FAA Certified Demo</span></H2>
          <p style={{ fontSize: 14, color: 'rgba(15,17,23,0.6)', lineHeight: 1.8, marginBottom: 32, maxWidth: 560 }}>
      ...
      <section style={{ background: DARK, padding: `${VPAD} 0` }}>
        <div style={{ maxWidth: W, margin: '0 auto', padding: `0 ${PAD}` }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <H2 light>WATCH IT<br /><span style={{ color: G }}>IN DETAIL</span></H2>
          </div>
          <div className="video-pair" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40 }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginBottom: 16, lineHeight: 1.65 }}>
      ...
      <section style={{ background: '#fff', padding: `${VPAD} 0` }}>
        <div style={{ maxWidth: W, margin: '0 auto', padding: `0 ${PAD}` }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <H2>CERTIFIED<br /><span style={{ color: G }}>EVERYWHERE IT MATTERS</span></H2>
          </div>
          <div className="cert-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16 }}>
            {[
              ...
              <div key={i} style={{ textAlign: 'center', padding: 'clamp(16px,2vw,24px) clamp(12px,1.5vw,16px)', background: BG, borderRadius: 12, border: '1.5px solid #e8f0e8' }}>
                <div style={{ width: 56, height: 56, borderRadius: '50%', background: G, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
                ...
                <p style={{ fontSize: 11, color: 'rgba(15,17,23,0.5)', lineHeight: 1.65 }}>{c.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: BG, padding: `${VPAD} 0` }}>
        <div style={{ maxWidth: W, margin: '0 auto', padding: `0 ${PAD}` }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <H2>WHAT'S IN<br /><span style={{ color: G }}>THE BOX</span></H2>
          </div>
          <div className="box-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, maxWidth: 960, margin: '0 auto' }}>
            {[
              ...
              { icon: '🔋', item: 'Rechargeable Battery', desc: 'Single or double (by variant)' },
              ...
              { icon: '📋', item: 'Warranty Card', desc: '2-year Sachdeva Medline warranty' },
            ].map((b, i) => (
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '16px', background: '#fff', borderRadius: 10, border: '1.5px solid #e8f0e8' }}>
                <span style={{ fontSize: 22, flexShrink: 0 }}>{b.icon}</span>
                <div>
                  <p style={{ fontSize: 13, fontWeight: 700, color: DARK, marginBottom: 3 }}>{b.item}</p>
                  <p style={{ fontSize: 11, color: 'rgba(15,17,23,0.45)', lineHeight: 1.5 }}>{b.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ background: '#fff', padding: `${VPAD} 0` }}>
        <div style={{ maxWidth: W, margin: '0 auto', padding: `0 ${PAD}` }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <H2>MORE<br /><span style={{ color: G }}>VIDEOS</span></H2>
          </div>
      ...
      <div ref={reviewsRef} style={{ background: BG, padding: `${VPAD} 0`, scrollMarginTop: 96 }}>
        <div style={{ maxWidth: W, margin: '0 auto', padding: `0 ${PAD}` }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            ...
            <H2>WHAT CUSTOMERS<br /><span style={{ color: G }}>ARE SAYING.</span></H2>
          </div>
      ...
      <div style={{ background: '#fff', padding: `${VPAD} 0` }}>
        <div style={{ maxWidth: W, margin: '0 auto', padding: `0 ${PAD}` }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            ...
            <H2>FREQUENTLY ASKED<br /><span style={{ color: G }}>QUESTIONS.</span></H2>
          </div>
      ...
      {/* ──── MOBILE STICKY CTA ──── */}
      <div className="mob-cta" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '2px solid #e8f0e8', padding: '10px 12px', zIndex: 500, boxShadow: '0 -4px 16px rgba(0,0,0,0.08)', display: 'none' }}>
        <div style={{ display: 'flex', gap: 8, maxWidth: 600, margin: '0 auto' }}>
          <button onClick={doAdd} disabled={adding} style={{ flex: '0 0 auto', padding: '13px 14px', background: '#fff', color: DARK, border: '2px solid #e8f0e8', borderRadius: 10, fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
            {adding ? '✓ ADDED' : 'ADD TO CART'}
          </button>
          <button onClick={doBuy} disabled={buying} style={{ flex: 1, background: G, color: '#fff', padding: '13px 12px', border: 'none', borderRadius: 10, fontSize: 12, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontFamily: 'inherit', boxShadow: '0 4px 14px rgba(61,170,53,0.3)', minWidth: 0 }}>
